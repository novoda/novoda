...
*It seems to me that the motivation for reference types and the example used to illustrate it are missing the point. Reference semantics is not about identity but about sharing, more especifically, unintended sharing. Value types are copied, reference types are shared. That's the main difference. For example, it seems to me that it's perfectly fine to have a person be represented by a struct.*
